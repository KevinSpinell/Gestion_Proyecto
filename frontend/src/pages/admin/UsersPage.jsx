...
// ── Reusable modal shell (same as rest of the app) ────────────────────────────
...
export default function UsersPage() {
  ...
  // RF-02 extended create form
  const [form, setForm]       = useState(EMPTY_TEACHER_FORM)

  // Edit form (legacy fields)
  ...
  const handleCreate = async () => {
    ...
    // Post-condition: reset form, show success banner, auto-close after 1.8s
    setForm(EMPTY_TEACHER_FORM)
    setSuccess(`✅ Profesor "${result.user.name}" registrado en la base de datos.`)
  ...
  const handleEdit = async () => {
    ...
    const updates = { name: editForm.name, email: editForm.email, estado: editForm.estado }
    if (editForm.password) updates.password = editForm.password

    if (selected.role === 'teacher') {
      // Build the payload the Teacher model understands
      const payload = { estado: editForm.estado }
      if (editForm.password) payload.clave = editForm.password

      try {
        const res = await fetch(`http://localhost:3001/api/teachers/${selected.id}`, {
          method: 'PUT',
    ...
    // Sync local state regardless (keeps table reactive)
          ...
          <div className="card-body">
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
              <div className="search-wrap" style={{ flex: 1 }}>
                ...
                <input className="form-input" placeholder="Buscar por nombre, documento, área…" value={search} onChange={e => setSearch(e.target.value)} />
              </div>
        ...
        >
          ...
          <div className="form-group">
            ...
            <input
              ...
              value={form.correo}
              onChange={set('correo')}
              placeholder="Ej: cmendez@institucion.edu"
            />
          </div>
          ...
          <div className="form-group">
            ...
            />
            <span className="form-hint">Se almacenará cifrada — no se mostrará de nuevo</span>
          </div>
          ...
          {selected.role === 'teacher' && (
            <div className="form-group">
              ...
              <select
                ...
                value={editForm.estado ? 'true' : 'false'}
                onChange={e => setEditForm({ ...editForm, estado: e.target.value === 'true' })}
              >
